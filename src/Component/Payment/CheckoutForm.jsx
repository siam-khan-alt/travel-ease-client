...
const CheckoutForm = ({ vehicle }) => {
  ...
  const { data: clientSecretData } = useQuery({
    queryKey: ["payment-intent", vehicle?._id, vehicle?.totalPrice],
    enabled: !!vehicle?.totalPrice && vehicle.totalPrice > 0,
    queryFn: async () => {
      const res = await instanceAxios.post("/create-payment-intent", {
    ...
    if (error) {
      ...
      const paymentInfo = {
        ...
        bookingDetails: {
          ...
          price: parseFloat(vehicle.totalPrice || vehicle.price),
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      >
        {processing
          ? "Processing..."
          : `Confirm Payment: $${vehicle?.totalPrice || vehicle?.price}`}
      </button>
    </form>
  );
};
