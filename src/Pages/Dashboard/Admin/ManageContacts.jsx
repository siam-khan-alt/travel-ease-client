...
import { FaHeadset, FaTrash, FaEnvelopeOpenText, FaReply, FaUser, FaClock } from 'react-icons/fa';
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black uppercase tracking-tighter text-gradient-gold flex items-center gap-3">
                        <FaHeadset className="text-[var(--primary)]" /> Concierge Desk
                    </h1>
                ...
                <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[var(--primary)] bg-[var(--primary)]/5 px-4 py-2 rounded-lg border border-[var(--primary)]/10">
                    Active: {contacts.length} Inquiries
                </div>
            </div>
            ...
            <div className="bg-[var(--card-bg)] border border-[var(--primary)]/10 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <table className="table w-full border-collapse">
                        ...
                        <tbody className="text-[var(--text-main)]">
                            {contacts.map((contact) => (
                                <tr key={contact._id} className="border-b border-white/5 hover:bg-[var(--primary)]/5 transition-all group">
                                    {/* Sender Column */}
                                    <td className="py-5 pl-8">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="font-bold uppercase tracking-wider text-xs">{contact.name}</div>
                                                <div className="text-[9px] opacity-40 lowercase mt-0.5">{contact.email}</div>
                                            </div>
                                        </div>
                                    </td>
                ...
                {contacts.length === 0 && (
                    <div className="text-center py-20 opacity-20">
                        ...
                        <p className="uppercase font-black tracking-widest text-sm">Inbox is empty</p>
                    </div>
                )}
            </div>
        </div>
