...
import { FaCarSide, FaChartLine, FaCubes, FaHistory, FaMoneyBillWave } from "react-icons/fa";
import StatCards from "../../../Component/Dashboard/Common/StatCards";
import BookingPieChart from "../../../Component/Dashboard/User/BookingPieChart"; // আপনার আগের প্রাইস চার্টটি এখানে ব্যবহার হবে
  ...
  return (
    <div className="space-y-10 animate-fade-in pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black uppercase tracking-tighter text-gradient-gold !text-center md:!text-left">Host Command Center</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-[var(--card-bg)] border border-[var(--primary)]/10 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-sm text-[var(--primary)] font-bold uppercase tracking-widest">Recent Fleet Requests</h3>
          </div>
                  ...
                  <tr key={idx} className="group">
                    <td className="py-4">
                      <div className="flex items-center gap-3">
                        <img src={activity.image} className="w-10 h-10 rounded-lg object-cover grayscale group-hover:grayscale-0 transition-all" alt="" />
                        <span className="text-xs font-bold uppercase">{activity.vehicleName}</span>
                      </div>
                    </td>
                    <td className="py-4 text-[11px] opacity-60 font-medium">{activity.userName}</td>
                    <td className="py-4 text-right text-xs font-black text-[var(--primary)]">${activity.price}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {!hostData?.recentActivity?.length && (
               <p className="text-center py-10 text-[10px] uppercase opacity-20 tracking-widest">No active deployments</p>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <BookingPieChart data={hostData?.recentActivity || []} />
          ...
          <div className="bg-gradient-to-br from-[#D4AF37]/10 to-transparent border border-[#D4AF37]/20 p-6 rounded-2xl relative overflow-hidden group">
             ...
             <p className="text-[10px] font-black uppercase tracking-widest text-[var(--primary)]">Fleet Strategy</p>
             <p className="text-xs mt-2 leading-relaxed opacity-60 italic">
               "Agent, your revenue flow is steady. Deploying high-tier luxury assets could increase your margin by 15%."
             </p>
          </div>
        </div>
      </div>
