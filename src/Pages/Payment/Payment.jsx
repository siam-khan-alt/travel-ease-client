...
const displayImage = vehicle?.image || vehicle?.coverImage || vehicle?.bookingDetails?.image;
const displayPrice = vehicle?.totalPrice || vehicle?.price;
  return (
    ...
    <img src={displayImage} className="rounded-2xl border border-[var(--primary)]/20 w-full h-48 object-cover" alt="" />
    <h3 className="text-xl font-bold text-[var(--primary)]">{vehicle?.vehicleName}</h3>
    <p className="text-2xl font-black text-[var(--primary)]">${displayPrice}</p>
  </div>
